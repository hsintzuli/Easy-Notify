...
const { pushNotification, getNotificationsByApp, deleteNotification, getNotificationById, checkUpdateAndDelete } = require('../../controllers/notifications');
const { userAuthentication, setChannelDataToReq, wrapAsync } = require('../../../utils/util');

router.use('/notifications', userAuthentication);
router.route('/notifications/:scheduledType').post(wrapAsync(setChannelDataToReq), wrapAsync(pushNotification));
